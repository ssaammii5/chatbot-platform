...
import { Settings, Save, Palette, Globe, Shield, Lock, Image as ImageIcon, User, Sliders } from 'lucide-react';
import { useState, useEffect } from 'react';
import { getStoredUser } from '../../lib/api';
import Sidebar from '../../components/Sidebar';

type TabId = 'general' | 'branding' | 'security' | 'profile' | 'platform' | 'preferences';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (user.role === 'super_admin') setActiveTab('platform');
      else if (user.role === 'admin') setActiveTab('general');
      else setActiveTab('profile'); // agent or supervisor
    }
  }, []);
  ...
  const getTabs = () => {
    if (userRole === 'super_admin') {
      return [{ id: 'platform', label: 'Platform Profile', icon: Globe } as const];
    }
    if (userRole === 'admin') {
      return [
        ...
        <div className="p-8 max-w-4xl mx-auto space-y-8 animate-fade-in">
          <div>
            <h1 className="text-3xl font-bold bg-gradient-to-r from-slate-100 to-slate-400 bg-clip-text text-transparent">
              {userRole === 'super_admin' ? 'Platform Settings' : userRole === 'admin' ? 'Tenant Settings' : 'Account Settings'}
            </h1>
            <p className="text-slate-400 mt-2">Manage your configuration and preferences.</p>
          </div>
              ...
              {activeTab === 'platform' && (
                <>
                  <div>
                    ...
                    <p className="text-sm text-slate-400 mt-1">Global settings for the multi-tenant system.</p>
                  </div>
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <input
                        type="text"
                        defaultValue="Aura Chat Platform"
              ...
              {activeTab === 'preferences' && (
                <>
                  <div>
                    ...
                    <p className="text-sm text-slate-400 mt-1">Customize your agent inbox experience.</p>
                  </div>
                  <div className="space-y-4">
                    <div className="space-y-2 flex items-center justify-between p-4 bg-slate-800/30 rounded-xl border border-[rgba(255,255,255,0.05)]">
                      <div>
                        <label className="text-sm font-medium text-slate-300">Desktop Notifications</label>
                        <p className="text-xs text-slate-500 mt-1">Receive alerts for new incoming chats.</p>
                      </div>
